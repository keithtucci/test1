...
//Rare exceptions to indicate a bug in the _myappeatize code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_myappeatize = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_myappeatize]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.myappeatize.module"
